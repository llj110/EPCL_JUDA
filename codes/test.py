...
def test_AD(model, data_loader, args, print_result=False, maxdice=0):
    ...
    logging.info('Dice:')
    logging.info('FL :%.2f(%.2f)' % (dice_mean[0], dice_std[0]))
    logging.info('TL :%.2f(%.2f)' % (dice_mean[1], dice_std[1]))
    logging.info('Mean :%.2f' % np.mean(dice_mean))

    logging.info('\nJaccard:')
    logging.info('FL :%.2f(%.2f)' % (jc_mean[0], jc_std[0]))
    logging.info('TL :%.2f(%.2f)' % (jc_mean[1], jc_std[1]))
    logging.info('Mask :%.2f' % np.mean(jc_mean))

    logging.info('\nHD95:')
    logging.info('FL :%.2f(%.2f)' % (hd95_mean[0], hd95_std[0]))
    logging.info('TL :%.2f(%.2f)' % (hd95_mean[1], hd95_std[1]))
    logging.info('Mask :%.2f' % np.mean(hd95_mean))

    logging.info('\nASSD:')
    logging.info('FL :%.2f(%.2f)' % (assd_mean[0], assd_std[0]))
    logging.info('TL :%.2f(%.2f)' % (assd_mean[1], assd_std[1]))
    logging.info('Mask :%.1f' % np.mean(assd_mean))

    logging.info('Inference time: %.2f' % ((time_end-time_start)/total_num))
...
def main():
    ...
    net_au = create_model(args)
    optimizer = optim.Adam(itertools.chain(net.parameters(), net_au.parameters()), lr=args.lr, betas=(args.beta1, args.beta2))
    # load_net_opt(net, optimizer, Path(args.load_path) / 'best.pth')
    load_net_opt1(net, net_au, optimizer, Path(args.load_path) / 'best.pth')
