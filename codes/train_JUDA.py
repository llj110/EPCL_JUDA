...
def sup_loss(predictions, labels, dice_loss, ce_loss, focal_loss, iou_loss, reliability_map=False):
    if torch.is_tensor(reliability_map) == False:
        ...
        loss_ce = (ce_loss(predictions[0], labels) * reliability_map).sum() / (reliability_map.sum() + 1e-16)
        loss_dice = dice_loss(predictions[1], labels, reliability_map)
        loss_focal = (focal_loss(predictions[2], labels) * reliability_map).sum() / (reliability_map.sum() + 1e-16)
        loss_iou = (ce_loss(predictions[3], labels)* reliability_map).sum() / (reliability_map.sum() + 1e-16)
        mean_prediction = (predictions[0] + predictions[1] + predictions[2] + predictions[3]) / 4
        loss_ce1 = (ce_loss(mean_prediction, labels) * reliability_map).sum() / (reliability_map.sum() + 1e-16)
        loss = (loss_ce + loss_dice + loss_focal + loss_iou) / 4 + loss_ce1

    return loss

def process_unlabeled_data(ema_net, unlab_img, args):
    # 1. 使用未增强的无标注数据生成预测
    ...
    # 4. 计算基于方差的不确定性
    var_uncertainty = unlab_ema_out_var.mean(dim=1)
    norm_var_uncertainty = var_uncertainty / var_uncertainty.sum(dim=(1, 2, 3), keepdim=True)

    # 5. 结合方差和熵的不确定性
    combined_uncertainty = torch.exp(-var_uncertainty) * (1 - entro_norm_uncertainty)
    reliability_map = combined_uncertainty / combined_uncertainty.sum(dim=(1, 2, 3), keepdim=True)

    # 6. 创建certain_mask
    certain_mask = torch.zeros_like(reliability_map).cuda()
    certain_mask[reliability_map > 0.9] = 1.0

    # certain_mask = certain_mask.unsqueeze(1)
    ...
    unlab_prototypes = getPrototype(unlab_ema_fts, unlab_ema_mask_onehot)

    return unlab_ema_out_pred, certain_mask, reliability_map, unlab_prototypes, unlab_ema_fts, unlab_ema_mask
    
def random_flip(unlab_img):
    if torch.rand(1).item() > 0.5:
        unlab_img = torch.flip(unlab_img, [3])  # 水平翻转
    if torch.rand(1).item() > 0.5:
        unlab_img = torch.flip(unlab_img, [2])  # 垂直翻转
    return unlab_img

def add_gaussian_noise(unlab_img, mean=0, std=0.1):
    noise = (torch.randn(unlab_img.size()) * std + mean).cuda()
    augmented_img = unlab_img + noise
    augmented_img = torch.clamp(augmented_img, 0, 1)
    return augmented_img

def random_rotation(unlab_img, degrees=15):
    batch_size, channels, depth, height, width = unlab_img.shape
    rotated_imgs = []

    for i in range(batch_size):
        img = unlab_img[i].cpu().numpy()
        angle = np.random.uniform(-degrees, degrees)
        rotated_img = np.zeros_like(img)
        for c in range(channels):
            rotated_img[c] = scipy.ndimage.rotate(img[c], angle, axes=(1, 2), reshape=False, mode='nearest')
        rotated_imgs.append(torch.tensor(rotated_img).cuda())

    return torch.stack(rotated_imgs)

def histogram_equalization(unlab_img):
    batch_size, channels, depth, height, width = unlab_img.shape
    equalized_imgs = []
    for i in range(batch_size):
        img = unlab_img[i].cpu().numpy()
        ...
        for c in range(channels):
            for z in range(depth):
                equalized_img[c, z, :, :] = cv2.equalizeHist((img[c, z, :, :] * 255).astype(np.uint8))
        equalized_imgs.append(torch.tensor(equalized_img / 255.0))
    return torch.stack(equalized_imgs).to(unlab_img.device)

def adjust_contrast(unlab_img, factor=0.2):
    mean = torch.mean(unlab_img)
    return (unlab_img - mean) * (1 + torch.randn(unlab_img.size()).cuda() * factor) + mean

def adjust_brightness(unlab_img, factor=0.2):
    return unlab_img * (1 + torch.randn(unlab_img.size()).cuda() * factor)

def gaussian_blur(unlab_img, kernel_size=3):
    batch_size, channels, depth, height, width = unlab_img.shape
    blurred_imgs = []
    for i in range(batch_size):
        img = unlab_img[i].cpu().numpy()
        ...
        for c in range(channels):
            blurred_img[c] = cv2.GaussianBlur(img[c], (kernel_size, kernel_size), 0)
        blurred_imgs.append(torch.tensor(blurred_img))
    return torch.stack(blurred_imgs).to(unlab_img.device)


def enhance_data(unlab_img):
    # Apply random crop and scale
    # unlab_img = random_crop_and_scale(unlab_img, output_size=(128, 128))

    # Apply random horizontal and vertical flip
    unlab_img = random_flip(unlab_img)

    # unlab_img = adjust_contrast(unlab_img)
    # unlab_img = adjust_brightness(unlab_img)

    # Apply random rotation
    # unlab_img = random_rotation(unlab_img, degrees=15)

    # Apply gaussian noise
    # unlab_img = add_gaussian_noise(unlab_img, mean=0, std=0.1)

    # unlab_img = gaussian_blur(unlab_img)
...
def main():
    ...
    for epoch in tqdm(range(args.num_epochs), ncols=70):
        ...
        for step, (labeled_batch, unlabeled_batch) in enumerate(zip(trainlab_loader, trainunlab_loader)):
            ...
            lab_img1, lab_img2 = lab_img[:args.lab_batch_size // 2], lab_img[args.lab_batch_size // 2:]
            lab_lab1, lab_lab2 = lab_lab[:args.lab_batch_size // 2], lab_lab[args.lab_batch_size // 2:]

            with torch.no_grad():
                img_mask, loss_mask = context_mask(lab_img1, args.mask_ratio)

            """Mix Input"""
            mix_img = lab_img1 * img_mask + lab_img2 * (1 - img_mask)
            mix_lab = lab_lab1 * img_mask + lab_lab2 * (1 - img_mask)

            lab_img = torch.cat((lab_img, mix_img), dim=0)
            lab_lab = torch.cat((lab_lab, mix_lab), dim=0)
            ...
            '''Unsupervised'''
            with torch.no_grad():
                enhanced_unlab_img = enhance_data(unlab_img)

                unlab_ema_out_pred_w, certain_mask, reliability_map, unlab_prototypes_w, unlab_ema_fts_w, unlab_ema_mask = process_unlabeled_data(ema_net, enhanced_unlab_img, args)
                unlab_ema_out_pred_s, _, _, unlab_prototypes_s, unlab_ema_fts_s, _ = process_unlabeled_data(ema_net, unlab_img, args)

                unlab_prototypes = [(unlab_prototypes_w[c] + unlab_prototypes_s[c]) / 2 for c in range(args.num_classes)]
                unlab_ema_fts = (unlab_ema_fts_s + unlab_ema_fts_w) / 2
            ...
            unlab_dist_w = torch.stack(
                [calDist(unlab_ema_fts_w, prototype, scaler=args.scaler) for prototype in prototypes], dim=1)
            unlab_dist_s = torch.stack(
                [calDist(unlab_ema_fts_s, prototype, scaler=args.scaler) for prototype in prototypes], dim=1)
            ...
            loss_pc_unlab_w = torch.sum(pixel_level_ce_loss(unlab_dist_w, unlab_ema_mask) * reliability_map)
            loss_pc_unlab_s = torch.sum(pixel_level_ce_loss(unlab_dist_s, unlab_ema_mask) * reliability_map)
            loss_pc_unlab = torch.sum(pixel_level_ce_loss(unlab_dist, unlab_ema_mask) * reliability_map)
            loss_pc = loss_pc_lab + consistency_weight * (loss_pc_unlab_w + loss_pc_unlab_s + loss_pc_unlab)

            loss_pr_w = torch.sum(pixel_level_ce_loss(unlab_ema_out_pred_w, unlab_ema_mask) * reliability_map)
            loss_pr_s = torch.sum(pixel_level_ce_loss(unlab_ema_out_pred_s, unlab_ema_mask) * reliability_map)
            loss_pr = (loss_pr_w + loss_pr_s) / 2

            '''Total'''
            loss = loss_supervised + loss_pc + loss_pr
            ...
            logging.info('epoch : %d, step : %d, loss_all: %.4f,'
                         ...
                         'loss_pc_unlab_w: %.4f, loss_pc_unlab_s: %.4f, train_dice: %.4f' % (
                             epoch + 1, step, loss.item(),
                             # loss_ce.item(), loss_dice.item(), loss_focal.item(), loss_iou.item(),
                             loss_supervised.item(), loss_pc_lab.item(), loss_pc_unlab_w.item(), loss_pc_unlab_s.item(), train_dice))
            ...
            writer.add_scalar('info/loss_pc_unlab_w', loss_pc_unlab_w, iter_num)
            writer.add_scalar('info/loss_pc_unlab_s', loss_pc_unlab_s, iter_num)
